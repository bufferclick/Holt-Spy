...
local function SafeDebugId(obj)
    ...
    return (ok and id and id ~= "") and id or tostring(obj)
end
...
DBtn("PC / Desktop", 0.28, function() isMobile = false end)
DBtn("Mobile",       0.72, function() isMobile = true  end)
...
local function ShowOverlay()
    ...
    TweenService:Create(Overlay, TweenInfo.new(0.18), {BackgroundTransparency = 0.6}):Play()
end
...
local function i2p(inst)
    ...
    local ok, res = pcall(function()
        ...
        if obj.Parent == game then
            local sOk, _ = pcall(function() return game:GetService(obj.ClassName) end)
            if sOk then
...
local function genScript(remote, args)
    ...
    if args and #args > 0 then
        ...
        if ok and res then
            ...
            local p = {}
            for _, a in ipairs(args) do p[#p+1] = v2s(a) end
            argsStr = "local args = {" .. table.concat(p, ", ") .. "}\n"
        end
    end
    if getnilReq then
    ...
    if remote:IsA("RemoteEvent") or remote:IsA("UnreliableRemoteEvent") then
        call = (args and #args > 0)
            and (rp .. ":FireServer(table.unpack(args))")
            or  (rp .. ":FireServer()")
...
local function enableHooks()
    ...
    spyActive = true
    TweenService:Create(TitleBtn, TweenInfo.new(0.25), {TextColor3 = T.Succ}):Play()
end
...
local function disableHooks()
    ...
    spyActive = false
    TweenService:Create(TitleBtn, TweenInfo.new(0.25), {TextColor3 = T.Err}):Play()
end
...
TitleBtn.MouseLeave:Connect(function()
    TweenService:Create(TitleBtn, TweenInfo.new(0.15), {
        TextColor3 = spyActive and T.Succ or T.Err
    }):Play()
end)
